...
from app.vensim.simulator import _scenario_contexts as vensim_scenario_contexts
...
def test_vensim_scenario_contexts_dedupes_baseline() -> None:
    scenarios = [
        ScenarioDefinition(id="baseline", name="Baseline", status="baseline"),
        ScenarioDefinition(id="policy_a", name="Policy A", status="policy"),
    ]
    contexts = vensim_scenario_contexts(scenarios, include_baseline=True)
    ids = [context[0] for context in contexts]
    assert ids == ["baseline", "policy_a"]
